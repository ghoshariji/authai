...
const StudentsScreen: React.FC = () => {
  ...
  const { data, isLoading, refetch, isFetching } = useGetStudentsQuery({ search, page, limit: 20 });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      Toast.show({ type: 'error', text1: 'Failed to remove student' });
    }
  ...
  const renderItem = ({ item }: { item: Student }) => (
    <View style={[styles.item, { backgroundColor: cardBg }, shadow.sm]}>
      <Avatar name={item.userId?.name ?? 'S'} uri={item.userId?.avatar} size={48} />
      <View style={styles.itemInfo}>
        <Text style={[styles.itemName, { color: textColor }]}>{item.userId?.name}</Text>
        <Text style={[styles.itemDetail, { color: secondaryColor }]}>Roll: {item.rollNumber}</Text>
        <Text style={[styles.itemDetail, { color: secondaryColor }]}>{item.department?.name ?? ''} • {item.class?.name ?? ''}</Text>
      </View>
      <View style={styles.itemActions}>
        <TouchableOpacity
          onPress={() => navigation.navigate('AddStudent', { studentId: item._id })}
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: bgColor }]}>
      <View style={styles.header}>
        <Text style={[styles.title, { color: textColor }]}>Students</Text>
        <TouchableOpacity
      ...
      <View style={styles.searchRow}>
        <SearchBar placeholder="Search students..." onSearch={handleSearch} isDark={isDark} />
      </View>
      <Text style={[styles.countText, { color: secondaryColor }]}>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              description={search ? `No results for "${search}"` : 'Add students to get started'}
      ...
      <ConfirmDialog
        ...
        message={`Remove ${deleteTarget?.userId?.name ?? 'this student'} from the college?`}
        confirmLabel="Remove"
